...
export default {
  ...
  async asyncData({ params, query }) {
    ...
    let firstCategory, secondCategory, lastCategory;

    const category = await API.getCategoryNewApi(
      `${params.category}/${params.subcategory}/${params.subtwocategory}`,
      1,
      "recommended"
    );
    firstCategory = category.categories.find(
      (el) => el.slug === params.category
    );
    secondCategory = firstCategory.children.find(
      (el) => el.slug === params.subcategory
    );
    lastCategory = secondCategory.children.find(
      (el) => el.slug === params.subtwocategory
    );
    let resCategory = lastCategory;
    ...
    const brudcrumns = [
      ...
      {
        text: firstCategory && firstCategory.name_ua,
        to: `/catalog/${firstCategory.slug}`,
      },
      {
        text: secondCategory && secondCategory.name_ua,
        to: `/catalog/${firstCategory.slug}/${secondCategory.slug}`,
      },
      {
        text: lastCategory && lastCategory.name_ua,
        to: `/catalog/${firstCategory.slug}/${secondCategory.slug}/${lastCategory.slug}`,
      },
    ];

    return {
      resCategory: resCategory && resCategory.children,
      products: category.pageOfItems,
      paginationData: category.pager,
      categoryData: resCategory,
      brudcrumns,
    };
  },
  components: {
  ...
  methods: {
    ...
    async paginationHandler(page) {
      ...
      const products = await API.getCategoryNewApi(
        `${this.$route.params.category}/${this.$route.params.subcategory}/${this.$route.params.subtwocategory}`,
        page,
        this.select
      );
