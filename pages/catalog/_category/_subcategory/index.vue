<template>
  <div class="catalog">
    ...
    <div class="container">
      ...
      <p v-if="products.length === 0" style="text-align: center;">На жаль товарів в цій категорії поки немає</p>
...
export default {
  ...
  async asyncData({ route, store, params, query, redirect, error }) {
    ...
    let parentCategory;
    let resCategory = category.categories.map((el) => {
      if (el.slug === params.category) {
        parentCategory = el;
      }
      const find = el.children.find((sub) => sub.slug === params.subcategory);
      if (find) return find;
    ...
    const brudcrumns = [
      ...
      {
          text: parentCategory && parentCategory.name_ua,
          to: `/catalog/${parentCategory.slug}`
      },
      {
        text: resCategory && resCategory[0].name_ua,
        to: `${route.path}/${resCategory[0].slug}`,
      },
    ];

    return {
      resCategory: resCategory ? resCategory[0].children : [],
